...
def gauss_KL(mu1, logstd1, mu2, logstd2):
    ...
    kl = torch.sum(logstd2 - logstd1 + (var1 + torch.square(mu1 - mu2))/(2*var2) - 0.5)
    return kl

def gauss_ent(mu, logstd):
    h = torch.sum(logstd + torch.constant(0.5*np.log(2*np.pi*np.e), torch.float32))
    return h
...
def visualize_pts_tb(writer, locations, latent_code, fig_key, iter=0):
    ...
    col_list = np.where(latent_code_num==0,'k',np.where(latent_code_num==1,'b','r'))
...
def load_data(data_f):
    ...
    X_all = np.concatenate(X_all).transpose((0,1, 3,2)).flatten().reshape(-1, 10)
